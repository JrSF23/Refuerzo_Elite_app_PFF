...
| XII.a Sin textos en código | Todo texto pasa por traducción, con español por defecto | **PASS (saldada el 2026-08-18)** | El idioma se resolvió en la rama `002-ui-espanol` y **el sistema de traducción quedó saldado en la feature 002-admin-frontend**: el frontend nuevo resuelve todos sus textos con una función `t()` propia contra un catálogo por dominio —sin librería de i18n—, y hay prueba automatizada de que añadir un segundo idioma no exige tocar ningún componente. En el backend, los cinco mensajes que quedaban escritos en controladores pasaron a `lang/es/`, y se añadió `lang/es/validation.php`, que no existía y hacía que toda la validación cayera al inglés. **Ya no queda ningún texto de usuario escrito en el código.** |
