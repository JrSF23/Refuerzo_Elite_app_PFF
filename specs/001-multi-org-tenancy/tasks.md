...
- [X] T107 Dar representación móvil propia a las tablas de `frontend/src/pages/Organizations/OrganizationsPage.jsx`, `frontend/src/pages/Organizations/OrganizationUsersPage.jsx` y `frontend/src/pages/UsersPage.jsx`, y verificarlo a 360 px (Principio VI). **Resuelto con el patrón `.stacked-table`**: por debajo de 768 px cada fila pasa a ser una tarjeta, con el intitulado de cada celda vía `data-label` y el encabezado oculto visualmente pero presente en el DOM para lectores de pantalla. **Verificado renderizando de verdad**, no por inspección del CSS: Chrome headless sobre un iframe de 360 px —Chrome impone 500 px mínimos de ventana, de ahí el iframe— da `scrollWidth == clientWidth` y **cero elementos desbordando**, incluso a 345 px efectivos cuando aparece la barra vertical
